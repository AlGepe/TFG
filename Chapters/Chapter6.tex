...
The mesaurements to which simulations will be compared where performed in Bilbao at the UPV-EHU laboratory.\iffalse PROBABLY NEED BETTER NAME\fi In the measurements the laser was moved vertically performing an scan in the Z coordinate as shown in the picture REFERENCE. This type of scanning measurement is commonly called \textit{Z-scan}. For each position in Z a waveform was recorded and later analysed. Each Z-scan was repeated for different \vias obtaining one Z-scan series of waveforms per \vias that will later be analysed together.
